...
<!--
  ...
  the built-in GITHUB_TOKEN, one committed TEXT cell per pinned repo — name,
  language and the repo's real description, fork honestly labelled, stars only
  when there are stars). No render-time image host is involved any more, so the
  block cannot break the way the old pin cards did. The marker names are load-bearing — the workflow replaces
